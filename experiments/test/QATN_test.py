...
def test_model():
    ...
    checkpoint_dir = os.path.join(checkpoint_dir, f"QATN/model_{datetime.today().strftime(fmt)}")
    ...
    fig.legend()
    fig.savefig("images/QATN/QATN_Test_Loss.png")
    plt.close()
    ...
    for i in range(num_horizons):
        ...
        fig.savefig(f"images/QATN/ScatterPlots_{i}.png", dpi=250)
        plt.close()
    ...
    for i in range(num_horizons):
        ...
        fig.suptitle(f"QATN Forecast - Horizon {horizons[i]//4}h")
        fig.legend(loc="upper right")
        fig.tight_layout()

        fig.savefig(f"images/QATN/predictions_{i}.png", dpi=300)
        plt.close()